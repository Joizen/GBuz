...
import { Routedata, Companydata, DPinroutedata } from '../../../models/datamodule.module'
    ...
    if (jsondata.code == "000") {
      ...
      if (this.maindata) {
        this.maindata.forEach(route => {
          ...
          console.log("SetrouteRoute route : ", route);
          if (listdp) {
            listdp.forEach(dp => {
              route.dpinroute.push(dp);
            });

          }
          else { route.dpinroute = []; }
        });
      }
    } else {
